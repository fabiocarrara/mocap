...
class MotionDataset(Dataset):
    def __init__(self, data_file, at_least=None, keep_actions=None, fps=120, offset='none', mapper=None):
        ...
        if mapper:
            ...
            descriptions = descriptions.groupby('Label').aggregate(
                {'Description': lambda x: os.path.commonprefix(x.tolist())})
        ...
        if self.multi:
            self.actions = Counter(
                a['action_id'] for s in self.data for a in s['annotations'] if mapper or a['action_id'] != 14)
        else:
